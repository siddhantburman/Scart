  ...
  return (
    <div className="w-[9%] h-[100%] shadow-xl shadow-black-900 flex flex-col border-none justify-between pb-2 border border-black rounded-md overflow-hidden">
      <div className="w-full h-[50%] p-7">
        <img
          className="w-full h-full transition-[0.7s] object-cover"
      ...
      <div className="w-full flex items-center justify-between pr-3 pl-4">
        ...
        {count > 0 ? (
          <button className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-bold border cursor-pointer border-white rounded-lg bg-blue-500 text-white transition-colors cursor-pointer">
            {" "}
            <RiSubtractLine
              onClick={() => {
                setCount(count - 1);
                props.setItems(props.items - 1);
                props.setPrice(
                  Number(props.price) - Number(props.details.price),
                );
              }}
              className="text-sm"
            />
          ...
          <button
            onClick={handleAdd}
            className="w-[80px] h-[32px] bg-blue-200 border border-blue-500 text-[17px] rounded-md"
          >
            ADD
          </button>
        )}
